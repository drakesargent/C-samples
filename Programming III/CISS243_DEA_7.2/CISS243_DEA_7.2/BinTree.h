...
// additional Binary Tree function that 
// takes an argument of a referenced object
// of the specified datatype.
template <class T>
void BinTree<T>::copyNode(T &item)
{
		TreeNode *nodePtr = root;

		while (nodePtr) // the tree is traversed
		{
			if (nodePtr->value == item) // if the node with the value matches the value of the passed item
			{
				item = nodePtr->value; // copy the entire value to the item
				break; // break out of loop
			}
			else if (item < nodePtr->value) // otherwise if the value is less, traverse left
			{
				nodePtr = nodePtr->left;
			}
			else // otherwise if the value is greater than, traverse right
			{
				nodePtr = nodePtr->right;
			}
		}
